...
def init(sMetro_xml_lib=""):
    ...
    if sMetro_xml_lib != "":
        ...
        metro_logger.print_init_message(metro_logger.LOGGER_INIT_MESSAGE, _("Auto configure METRo XML library"))
        try:
            metro_util.test_import("toolbox.metro_xml_libxml2")
            ...
            metro_logger.print_init_message(metro_logger.LOGGER_INIT_SUCCESS, _("metro_xml_libxml2 will be used."))
            metro_xml_libxml2 = metro_util.import_name('toolbox', "metro_xml_libxml2")
...
def extract_xpath(lDefs, dReadHandlers, domDom, sXpath, bIs_matrix=False):
    """
        ...
        Arguments: [I] lDefs: list of dictionaries containing the data definitions as defined in metro_config.py
                   [I] dReadHandlers: dictionary of read handler.
                    ...
                    [I] sXPath: Path of the node containing all the subnodes (header, prediction, etc.)
                    [I] bIs_matrix: Boolean indicating if there is more than one value to fetch
    ...
    node_items = metro_xml_lib.xpath(metro_xml_lib.get_dom_root(domDom), sXpath)
    if bIs_matrix:
        if len(node_items) > 0:
            llExtractedValue = []
...
def extract_data(lDefs, dReadHandlers, nodeItems):
    """
        ...
        Arguments:  [I] lDefs: list of dictionary containing the definition of XML element in metro_config.py
                    [I] dReadHandlers: dictionary of read handler.
                                       key = data type name (ex: INTEGER),
                                       value = handler (ex: toolbox.metro_dom2metro_handler.read_integer)
                    [I] nodeItems: fetch the elements in theses nodes
    ...
    for dDef in lDefs:
        ...
        if 'CHILD' in dData_type[sData_type_name]:
...
def extract_data_from_node(sTag, nodeBranch):
    nodeLeaf = metro_xml_lib.xpath(nodeBranch, sTag + "/text()")
    if nodeLeaf:
        sRslt = metro_xml_lib.get_node_value(nodeLeaf[0])
    else:
        sRslt = None
...
def create_node_tree_from_dict(domDoc, nodeParent, lDefs, dWriteHandlers, dData):
    # Retrieve the information on the data types
    dStandard_data_type = metro_config.get_value('XML_DATATYPE_STANDARD')
    dExtended_data_type = metro_config.get_value('XML_DATATYPE_EXTENDED')
    dData_type = metro_util.join_dictionaries(dStandard_data_type, dExtended_data_type)
    ...
    for dDef in lDefs:
        ...
        if 'CHILD' in list(dData_type[sData_type_name].keys()):
...
def create_node_tree_from_matrix(domDoc, nodeParent, sPrediction_xpath, lDefs,
    ...
    """
    # Retrieve the information about the data types
    dStandard_data_type = metro_config.get_value('XML_DATATYPE_STANDARD')
    dExtended_data_type = metro_config.get_value('XML_DATATYPE_EXTENDED')
    dData_type = metro_util.join_dictionaries(dStandard_data_type, dExtended_data_type)
    ...
    for npData in npMatrix:
        ...
        if sPrediction_xpath is not None and sPrediction_xpath != "":
...
def get_handler(sHandlerType, dDefData, dAllDefData=None):
    if dAllDefData is None:
        # Retrieve the information about the data types
        dStandard_data_type = metro_config.get_value('XML_DATATYPE_STANDARD')
        dExtended_data_type = metro_config.get_value('XML_DATATYPE_EXTENDED')
        dAllDefData = metro_util.join_dictionaries(dStandard_data_type, dExtended_data_type)

    sTag = dDefData['NAME']
    sXml_tag = dDefData['XML_TAG']
    sData_type_name = dDefData['DATA_TYPE']

    if sData_type_name not in list(dAllDefData.keys()):
        sMessage = _("Invalid data_type: (%s) for the following ") % sData_type_name + \
                   _("tag:(%s). Default data type will be used.") % sTag
        metro_logger.print_message(metro_logger.LOGGER_MSG_WARNING, sMessage)
