...
"""
    ...
    Notes: Time zone string used through this file correspond to the code used by the OS.
           The list can be found in the directory usr/share/zoneinfo on your system.
"""
...
def parse_date_string(sDate):
    ...
    if sDate is not None:
        try:
            ...
            sMessage = _("The following error occured when parsing the ") + _("ISO 8601 date:\n %s") % sError
            fDate = metro_constant.NaN
            raise metro_error.Metro_date_error(sMessage)
    else:
        sMessage = _("The following error occurred when parsing a ") + _("date:\nNo date string to convert")
        raise metro_error.Metro_date_error(sMessage)
    return fDate
...
def time_zone_convert(tm, source_zone, dest_zone):
    """
        Convert a broken-down time (time.struct_time or tuple) from one named time zone to another.
    """
...
def get_elapsed_time(ftime1, ftime2, sTimeZone="UTC", sUnit="hours"):
    """
        ...
        Parameters.
                    ...
                    sTimeZone (string): time zone code. See 'Note' in the general comment of metro_util.
                    sUnit (string): Precision of which this function should round.
                    Possible choices are in ['hours', 'days', 'seconds']
    ...
    if sUnit == "hours":
        ...
        nNbrDays = cTimeDifference.days + cTimeDifference.seconds / 3600.0 * 1 / 24.0
        return nNbrDays
    elif sUnit == "seconds":
        nNbrSeconds = ftime1 - ftime2
        return nNbrSeconds
    else:
        sInvalidParameterError = _("Invalid criteria in get_elapsed_time: sUnit = %s") % sUnit
        raise metro_error.Metro_date_error(sInvalidParameterError)


def tranform_decimal_hour_in_minutes(fTimeHour):
    """
        Return an array containing the hour, the minutes and the seconds, respectively.
...
def get_iso_8601(nYear, nMonth, nDay, nHour=0, nMinute=0, nSecond=0):
    ...
    if nHour == nMinute == nSecond == 0:
        ...
        sIso = sYear + '-' + sMonth + '-' + sDay + 'T' + sHour + ':' + sMinute + ':' + sSecond + 'Z'
    return sIso
...
def str_to_at_least_two_digits(nNumber):
    """
        ...
        There must be a 'regular' way to do it in python but it takes longer to find it than to write it.
    """
    sRes = str(nNumber)
    if len(sRes) < 2:
        sRes = '0' + sRes
...
def in_the_dark(nCurrentTime, fSunrise, fSunset):
    """
        Sometimes, value returned by Sun.py are over 24. Since the time of day is needed with modulo 24,
        a special check must be performed. See https://gna.org/bugs/?8277 for more details.
    ...
    """
    bDark = False
    if ((fSunset % 24 > fSunrise % 24) and (nCurrentTime < fSunrise or nCurrentTime > fSunset)) or \
            (not (fSunset % 24 > fSunrise % 24) and (fSunset % 24 < nCurrentTime < fSunrise % 24)):
        bDark = True
    return bDark
...
def get_eot(fTime, fLat):
    """
        Subroutine computing the part of the equation of time needed in the computing of the theoritical solar flux
    ...
    # in minutes
    fEot = 0.002733 - 7.343 * sin(fA) + .5519 * cos(fA) - 9.47 * sin(2.0 * fA) - 3.02 * cos(2.0 * fA) - \
           0.3289 * sin(3. * fA) - 0.07581 * cos(3.0 * fA) - 0.1935 * sin(4.0 * fA) - 0.1245 * cos(4.0 * fA)
...
def solcons(dAlf):
    """
        Statement function that calculates the variation of the solar constant as a function of the julian day.
        (dAlf, in radians)
