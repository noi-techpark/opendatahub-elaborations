...
def genxml_tag(sTag_name, lAttribute=None):
    if lAttribute is None:
        lAttribute = []
    sTag = ""
    sTag += "<" + sTag_name
    if len(lAttribute) > 0:
...
def genxml_public_catalog_entry(sPublic_id, sUri):
    lAtt = [("publicId", sPublic_id), ("uri", sUri)]
...
def generate_dtd_catalog():
    ...
    sCatalog += genxml_public_doctype("-//OASIS//DTD Entity Resolution XML Catalog V1.0//EN",
                                      "http://www.oasis-open.org/committees/entity/release/1.0/catalog.dtd")
    sCatalog += "\n"

    sCatalog += genxml_tag("catalog", [("xmlns", "urn:oasis:names:tc:entity:xmlns:xml:catalog")])
    sCatalog += "\n"

    sMetro_root_path = metro_util.get_metro_root_path()
    sMetro_catalog_path = sMetro_root_path + "/usr/share/metro/data/DTD/"

    sCatalog += genxml_public_catalog_entry("-//Apple Computer//DTD PLIST 1.0//EN", sMetro_catalog_path + "plist.dtd")

    sCatalog += genxml_public_catalog_entry("-//METRo//DTD FORECAST 1.0//EN", sMetro_catalog_path + "forecast.dtd")

    sCatalog += genxml_public_catalog_entry("-//METRo//DTD OBSERVATION 1.0//EN",
                                            sMetro_catalog_path + "observation.dtd")

    sCatalog += genxml_public_catalog_entry("-//METRo//DTD STATION 1.0//EN", sMetro_catalog_path + "station.dtd")
    ...
    try:
        ...
        fCatalog = None
        sError_message = _("can't create METRo DTD catalog file:'%s'") % sCatalog_filename
        metro_logger.print_init_message(metro_logger.LOGGER_INIT_ERROR, sError_message)
    else:
        ...
        metro_logger.print_init_message(metro_logger.LOGGER_INIT_SUCCESS, sSuccess_message)
