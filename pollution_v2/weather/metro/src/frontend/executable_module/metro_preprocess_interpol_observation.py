...
"""
    ...
    Description: Interplation of data in order to be at every 30 seconds.
                Solar flux is a particular case and is handle in metro_preprocess_fsint2.py .
...
class Metro_preprocess_interpol_observation(Metro_preprocess):
    ...
    def start(self):
        ...
        try:
            self.__set_time(observation_data.get_controlled_data(), observation_data.get_interpolated_data(),
                            observation_data)
            self.__interpolate_AT(observation_data.get_controlled_data(), observation_data.get_interpolated_data())
            self.__interpolate_TD(observation_data.get_controlled_data(), observation_data.get_interpolated_data())
            self.__interpolate_WS(observation_data.get_controlled_data(), observation_data.get_interpolated_data())
            self.__interpolate_ST(observation_data.get_controlled_data(), observation_data.get_interpolated_data())
            self.__interpolate_SST(observation_data.get_controlled_data(), observation_data.get_interpolated_data())
            self.__interpolate_PI(observation_data.get_controlled_data(), observation_data.get_interpolated_data())
            self.__interpolate_SC(observation_data.get_controlled_data(), observation_data.get_interpolated_data())
            self.__interpolate_validatation(observation_data.get_controlled_data(), observation_data)
        except metro_error.Metro_input_error as inst:
            metro_logger.print_message(metro_logger.LOGGER_MSG_INFORMATIVE, str(inst))
        pObservation.set_data_collection(observation_data)

    def __set_time(self, ro_controlled_data, ro_interpolated_data, observation_data):
        """
            Name: __set_time
        ...
        self.npTimeInterpolated = numpy.arange(npTime[0], npTime[len(npTime) - 1], metro_constant.fTimeStep)

        # 1.1574e-5 is the conversion from seconds to day.
        #  1/(24*3600) = 1.1574e-5
        self.npTimeInterpolated = self.npTimeInterpolated / 3600 - 24 * ((self.npTimeInterpolated * 1.1574e-5).
                                                                         astype(numpy.int32))

        # Save the time array
        if len(self.npTimeInterpolated) > 1:
            ro_interpolated_data.append_matrix_col('Time', self.npTimeInterpolated)
        # Only one observation, abort interpolation
        elif len(self.npTimeInterpolated) == 1:
            observation_data.set_attribute('NO_OBS', [False, False, False, True])
            raise metro_error.Metro_input_error(OneObservationException)
        else:
            observation_data.set_attribute('NO_OBS', [True, True, True, True])
            sMessage = _("No valid observation in: ") + metro_config.get_value('FILE_OBSERVATION_FILENAME')
    ...
    def __interpolate_AT(self, ro_controlled_data, ro_interpolated_data):
        """
            ...
            Parameters: [I] metro_data ro_controlled_data : controlled data. Read-only
                        [I] metro_data ro_interpolated_data : container of the interpolated data.
    ...
    def __interpolate_WS(self, ro_controlled_data, ro_interpolated_data):
        """
            ...
            Description: Does the interpolation of the wind speed.
                         Wind is in km/h and is converted in m/s by the product with 0.2777777
    ...
    def __interpolate_SC(self, ro_controlled_data, ro_interpolated_data):
        """
            ...
            Description: Does the interpolation of the Road Condition 33 is the SSI code.
                         SEE the documentation for the conversion between this code and the other standard:
    ...
    def __interpolate_validatation(self, ro_controlled_data, observation_data):
        """
            ...
            Description: Does the interpolation of all the attributes that were set in metro_preprocess_qa_qc_observation
        """
        npTimeOrig = ro_controlled_data.get_matrix_col('Time')
