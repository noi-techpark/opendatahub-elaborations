...
"""
    ...
    Description:   Module that is used to validate the well-formation of the XML files.
        ...
        if self.infdata_exist(sInfdata_tag):
            ...
            if sXml_content != "":
                try:
                    ...
                    sMessage = _("Fatal Error when validating %s ") % sInfdata_tag + \
                               _("XML string.\nThe error is:\n%s") % (str(inst))
                    metro_logger.print_message(metro_logger.LOGGER_MSG_STOP, sMessage)
                else:
                    sMessage = _("%s XML string has been validated") % sInfdata_tag
                    metro_logger.print_message(metro_logger.LOGGER_MSG_INFORMATIVE, sMessage)
            else:
                sMessage = _("Fatal Error, %s XML string is empty") % sInfdata_tag
                metro_logger.print_message(metro_logger.LOGGER_MSG_STOP, sMessage)
        else:
            sMessage = _("Fatal Error, no %s XML string to validate.") % sInfdata_tag
            metro_logger.print_message(metro_logger.LOGGER_MSG_STOP, sMessage)

    # Private method
    def __validate_xml_string(self, sXml_content):
        if sXml_content is not None:
            try:
