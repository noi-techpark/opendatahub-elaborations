    ...
    def start(self):
        ...
        if metro_config.key_exist('FILE_FORECAST_IN_FILENAME'):
            ...
            try:
                ...
                infdata_forecast = metro_infdata.Metro_infdata('FORECAST', metro_infdata.DATATYPE_METRO_DATA_COLLECTION)
                infdata_forecast.set_input_information(sFile_content)
                self.add_infdata(infdata_forecast)
        else:
            sError_message = _("METRo need a forecast file, please use the ") + _("option: '--input-forecast'")
            metro_logger.print_message(metro_logger.LOGGER_MSG_STOP, sError_message)
