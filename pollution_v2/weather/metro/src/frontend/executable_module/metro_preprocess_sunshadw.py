...
"""
    ...
    For given location on the Earth and for given time moments corrects solar flux by taking into account shielding
    of the Sun by visible horizon.
"""


import math
from external_lib import sunpos
...
def get_horiz_elev(azim, horiz):
    """
        ...
        As the horizon is discrete, interpolation is necessary to get elevation for arbitrary azimuth.
        Fast linear interpolation method is implemented which works in normalized units (no searching required).
        However, it requires horizon (azim, elev) pairs being ordered by growing azimuth values and also uniform step
        in azimuth (i.e, neighbour azimuth values are displaced by same distance), otherwise it can return
        a wrong elevation values. Note that uniform azimuth is assumed but not checked. The azim value is assumed
        within the maximum range 0 to 360 degree, or within the range of horizon azimuth values (horizon must not
        cover full 360 degree range) but it is not checked. In future, other (more accurate) interpolation methods
        can be implemented allowing for horizon with non-uniform azimuth step.
