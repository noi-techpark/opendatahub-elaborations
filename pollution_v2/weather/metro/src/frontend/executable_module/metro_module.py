...
class Metro_module:
    ...
    INFDATATYPE_DATA_COLLECTION = metro_infdata.DATATYPE_METRO_DATA_COLLECTION
    ...
    def start(self):
        ...
        metro_logger.print_blank_line(metro_logger.LOGGER_MSG_EXECPRIMARY)
        sMessage = _("%s module: start execution") % (self.__module__.upper())
        metro_logger.print_message(metro_logger.LOGGER_MSG_EXECPRIMARY, sMessage)

    def stop(self):
        sMessage = _("%s module: end of execution") % (self.__module__.upper())
        metro_logger.print_message(metro_logger.LOGGER_MSG_EXECPRIMARY, sMessage)

    def receive_data(self, infdata_container):
        sMessage = _("%s module: receiving data") % (self.__module__.upper())
        metro_logger.print_message(metro_logger.LOGGER_MSG_EXECSECONDARY, sMessage)
        self.infdata_container = infdata_container

    def send_data_to(self, target_module):
        sMessage = _("%s module:\nsending data to module %s") % \
                   (self.__module__.upper(), target_module.__module__.upper())
        metro_logger.print_message(metro_logger.LOGGER_MSG_EXECSECONDARY, sMessage)
        target_module.receive_data(self.infdata_container)

    def get_receive_type(self):
        if self.__class__ is not Metro_module:
            sMessage = _("class %s misses method 'get_receive_type'") % str(self.__class__)
            raise NotImplementedError(sMessage)

    def get_send_type(self):
        if self.__class__ is not Metro_module:
            sMessage = _("class %s misses method 'get_send_type'") % str(self.__class__)
            raise NotImplementedError(sMessage)
    ...
    def get_xml_format(self, sContainer_id):
        if self.infdata_container.infdata_exist(sContainer_id):
            return self.infdata_container.get_infdata(sContainer_id).get_xml_format()
    ...
    def get_input_information(self, sContainer_id):
        if self.infdata_container.infdata_exist(sContainer_id):
            return self.infdata_container.get_infdata(sContainer_id).get_input_information()
    ...
    def get_output_information(self, sContainer_id):
        if self.infdata_container.infdata_exist(sContainer_id):
            return self.infdata_container.get_infdata(sContainer_id).get_output_information()
