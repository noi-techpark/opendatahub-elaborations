...
"""
    ...
    Description: Subroutine correcting the incident solar flux to take the sunrise/sunset into account.

    Notes: Take a special care with the difference between the interpolated values and the raw one.
           The search for the sunrise/sunset is performed on the raw values of SF.
           The number of time step is performed on the raw values of time. The number of time steps should
           normally correspond to the interpolated intervals.
"""
...
import metro_logger
import metro_config
from executable_module import metro_preprocess_sunshadw
...
class Metro_preprocess_fsint2(Metro_preprocess):
    ...
    def start(self):
        ...
        self.__set_attribute(forecast_data.get_controlled_data(), forecast_data.get_interpolated_data(), station_data)
        self.__print_info()
        # SF
        self.__set_sf(forecast_data.get_controlled_data(), forecast_data.get_interpolated_data())
        # IR
        self.__set_ir(forecast_data.get_controlled_data(), forecast_data.get_interpolated_data())
        self.__set_cc(forecast_data.get_controlled_data(), forecast_data.get_interpolated_data())
        pForecast.set_data_collection(forecast_data)
        pStation.set_data(station_data)

    def __set_attribute(self, wf_controlled_data, wf_interpolated_data, station_data):
        """
    ...
    def __print_info(self):
        """
            Print the information about the sunrise/sunset computed for this day and emplacement.
        """
        if self.fLon < 0:
            sLon = 'W'
        else:
        ...
        if self.fLat > 0:
            ...
            sLat = 'S'
        tSunset = metro_date.tranform_decimal_hour_in_minutes(self.fSunset)
        tSunrise = metro_date.tranform_decimal_hour_in_minutes(self.fSunrise)
        sMessage = _("For the date %d-%d-%d,\n") % (self.nStartDay, self.nStartMonth, self.nStartYear) + \
                   ...
                   _("sunset  is at %d:%d:%d UTC") % (tSunset[0], tSunset[1], tSunset[2])
    ...
    def __set_sf(self, wf_controlled_data, wf_interpolated_data):
        ...
        if metro_config.get_value('SF'):
            ...
            if (self.infdata_exist('HORIZON')) and (self.npAzim is not None):
                ...
                npSF2 = metro_preprocess_sunshadw.get_corrected_solar_flux(tDate2, npSF2, self.fLat, self.fLon,
                                                                           list(zip(self.npAzim, self.npElev)),
                                                                           m=sunshadw_method)
        ...
        npForecastedTime = wf_controlled_data.get_matrix_col('FORECAST_TIME')
        fStartForecastTime = npForecastedTime[0]
        # Get solar fluxes for this cloud cover for this specific day
        npSF = metro_physics.get_sf(npCloudOctal, npTimeHour, npForecastedTime, self.fSunrise, self.fSunset,
                                    self.fLat, self.fLon)
        npSF2 = metro_util.interpolate(npTime, npSF)
    ...
    def __set_sunrise_sunset(self, wf_controlled_data):
        """
            Description: Get the value of sunrise/sunset for the first day of forecast.
        ...
        """
        ctimeFirstForecast = wf_controlled_data.get_matrix_col('FORECAST_TIME')[0]
        ...
        cSun = Sun.Sun()
        (fSunriseTimeUTC, fSunsetTimeUTC) = cSun.sunRiseSet(self.nStartYear, self.nStartMonth, self.nStartDay,
                                                            self.fLon, self.fLat)
