...
class W3CDate:
    ...
    def getISO8601(self):
        ...
        if self.timezone is None:
            ...
            if isinstance(-self.timezone, int):
                result = result + (' +%02d%02d' % divmod(-self.timezone // 60, 60))
            elif isinstance(-self.timezone, float):
                result = result + (' +%02d%02d' % divmod(-self.timezone / 60, 60))
        else:
            if isinstance(-self.timezone, int):
                result = result + (' +%02d%02d' % divmod(-self.timezone // 60, 60))
    ...
    def _check(self):
        ...
        if int(self.dateSecs) != int(origSecs):
            print("Error: dateSecs don't match (%f became %f)" % (origSecs, self.dateSecs))
        origMiniTuple = self.dateMiniTuple
        self.dateMiniTuple = self.dateFullTuple = None
        self._cacheTuples()
        if self.dateMiniTuple != origMiniTuple:
            print("Error: dateMiniTuples don't match (%s became %s)" % (repr(origMiniTuple), repr(self.dateMiniTuple)))
