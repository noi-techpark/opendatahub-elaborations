...
class ValidationEntry:
    ...
    validation_class: ValidationTypeClass
    validation_value: Optional[float]
    period: Optional[int]
    ...
    def get_data_types() -> List[DataType]:
        ...
        for vehicle in VehicleClass:
            for validation_type in ValidationTypeClass:
                data_types.append(DataType(f"{vehicle.name}-{validation_type.name}", f"{vehicle.value} is {validation_type.name}", "Validation", "-", {}))
        return data_types
...
class ValidationMeasureCollection(MeasureCollection[ValidationMeasure, TrafficSensorStation]):
    ...
    @staticmethod
    def build_from_validation_entries(validation_entries: List[ValidationEntry], provenance: Provenance) -> ValidationMeasureCollection:
        """
        ...
        """
        data_types_dict: Dict[str, DataType] = {data_type.name: data_type for data_type in ValidationMeasure.get_data_types()}
        ...
        for validation_entry in validation_entries:
            validation_measures.append(ValidationMeasure(
                ...
                value=validation_entry.validation_value
            ))
